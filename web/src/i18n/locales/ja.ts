const ja = {
  translation: {
    ...
    image: {
      ...
      diskShared:
        '仮想ディスクが有効なため、このデバイスの /data は読み取り専用です。イメージを削除またはアップロードするには仮想ディスクを無効にしてください。',
    ...
    settings: {
      ...
      device: {
        ...
        diskBreakDownload:
          '仮想ディスクを有効にすると、イメージのダウンロードが中断されます。続行しますか？',
        network: '仮想ネットワークカード',
      ...
      netbird: {
        ...
        update: '更新',
        updateConfirm: 'NetBird を今すぐ更新しますか？',
        updateWarning:
          '置き換えの間クライアントは停止します。NetBird 経由で接続している場合、その接続は切断され、新しいクライアントの起動後に復旧するはずです。',
        updateAvailable: 'このファームウェアにはより新しい NetBird が含まれています',
        updateHint:
          'インストール済みのクライアントはそのまま使用できます。更新すると、このファームウェアに含まれるバージョンに置き換わります。',
        version: 'バージョン',
        disconnect: '切断',
