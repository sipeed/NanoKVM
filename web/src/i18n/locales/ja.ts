    ...
    settings: {
      ...
      network: {
        ...
        ip: {
          ...
          savedReconnect: '保存しました。接続が切断されます — {{url}} で NanoKVM を再度開いてください',
          warning: 'IP アドレスを変更すると現在のセッションが切断されます。誤った値を設定すると Web インターフェースにアクセスできなくなる可能性があります（復旧には SD カード/物理アクセスが必要）。アドレス、マスク、ゲートウェイが正しいことを確認してください。',
          confirmTitle: 'IP アドレスを変更しますか？',
          confirmStatic: '有線接続がただちに再構成され、このセッションは切断されます。その後 {{url}} で NanoKVM を開いてください。続行しますか？',
