const ru = {
  translation: {
    ...
    image: {
      ...
      diskOnWarn:
        'Монтирование образа отключает виртуальный диск, и компьютер теряет доступ к /data. Продолжить?',
      diskOffWarn:
        'Виртуальный диск отключён, его нужно включить для монтирования образа. USB-устройства компьютера будут переподключены. Продолжить?',
      diskShared:
        'Виртуальный диск включён, поэтому /data здесь только для чтения. Отключите его, чтобы удалять или загружать образы.',
    ...
    download: {
      ...
      diskShared: 'Сначала отключите виртуальный диск, затем используйте загрузку образов.',
      diskOff: 'Отключить виртуальный диск',
    ...
    settings: {
      ...
      device: {
        ...
        diskLocked: 'Отключён, пока смонтирован образ (включение отменит монтирование)',
        diskCancelMount:
          'Включение виртуального диска отменит смонтированный образ и прервёт текущую загрузку или выгрузку образа. Продолжить?',
        diskBreakDownload: 'Включение виртуального диска прервёт загрузку образов. Продолжить?',
      ...
      netbird: {
        ...
        update: 'Обновить',
        updateConfirm: 'Обновить NetBird сейчас?',
        updateWarning:
          'На время замены клиент останавливается. Если вы подключены через NetBird, это соединение прервётся и должно восстановиться после запуска нового клиента.',
        updateAvailable: 'С этой прошивкой поставляется более новый NetBird',
        updateHint:
          'Установленный клиент продолжает работать. Обновление заменит его версией, которая поставляется с этой прошивкой.',
        version: 'Версия',
