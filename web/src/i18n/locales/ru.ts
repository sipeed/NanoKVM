    ...
    settings: {
      ...
      network: {
        ...
        ip: {
          ...
          savedReconnect: 'Сохранено. Соединение сейчас прервётся — снова откройте NanoKVM по адресу {{url}}',
          warning: 'Изменение IP-адреса разорвёт текущий сеанс. Неверное значение может заблокировать доступ к веб-интерфейсу (для восстановления потребуется SD-карта/физический доступ). Убедитесь, что адрес, маска и шлюз указаны верно.',
          confirmTitle: 'Изменить IP-адрес?',
          confirmStatic: 'Проводное подключение будет немедленно перенастроено, и текущий сеанс прервётся. Затем откройте NanoKVM по адресу {{url}}. Продолжить?',
