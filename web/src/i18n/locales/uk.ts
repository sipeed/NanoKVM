    ...
    settings: {
      ...
      network: {
        ...
        ip: {
          ...
          savedReconnect: "Збережено. З'єднання зараз розірветься — знову відкрийте NanoKVM за адресою {{url}}",
          warning: 'Зміна IP-адреси розірве поточний сеанс. Неправильне значення може заблокувати доступ до веб-інтерфейсу (для відновлення знадобиться SD-карта/фізичний доступ). Переконайтеся, що адреса, маска та шлюз правильні.',
          confirmTitle: 'Змінити IP-адресу?',
          confirmStatic: 'Дротове підключення буде негайно переналаштовано, і цей сеанс розірветься. Потім відкрийте NanoKVM за адресою {{url}}. Продовжити?',
